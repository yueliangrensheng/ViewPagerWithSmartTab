...
package com.yazao.lib.smartlayout;
...
class SmartTabStrip extends LinearLayout {
	...
	SmartTabStrip(Context context, AttributeSet attrs) {
		...
		mIndicationInterpolator = SmartTabIndicationInterpolator
	...
	protected void onDraw(Canvas canvas) {
		...
		final SmartTabLayout.TabColorizer tabColorizer = mCustomTabColorizer != null ? mCustomTabColorizer
		...
		if (childCount > 0) {
			...
			if (mSelectionOffset > 0f
				...
				float leftOffset = mIndicationInterpolator
						.getLeftEdge(mSelectionOffset);
				float rightOffset = mIndicationInterpolator
						.getRightEdge(mSelectionOffset);
				float thicknessOffset = mIndicationInterpolator
						.getThickness(mSelectionOffset);

				View nextTitle = getChildAt(mSelectedPosition + 1);
				left = (int) (leftOffset * nextTitle.getLeft() + (1.0f - leftOffset)
						* left);
				right = (int) (rightOffset * nextTitle.getRight() + (1.0f - rightOffset)
						* right);
				thickness = thickness * thicknessOffset;
			}

			mIndicatorPaint.setColor(color);
			mIndicatorRectF.set(left, height - (mIndicatorThickness / 2f)
					- (thickness / 2f), right, height
					- (mIndicatorThickness / 2f) + (thickness / 2f));

			if (mIndicatorCornerRadius > 0f) {
				canvas.drawRoundRect(mIndicatorRectF, mIndicatorCornerRadius,
